...
import messages from '../OrderBuy/messages';
...
function OrderSell({ currency, symbol }) {
  ...
  const { availableBalance, makerFeeRate } = account;
  useEffect(() => {
    if (currency) {
      setInitData();
    }
  ...
  const calAmount = () => {
    ...
    const feesGet = accountFloor * (makerFeeRate / 100);
  ...
  const setInitData = async () => {
    ...
    try {
      ...
      const result = await request(`/api/account/${symbol}`, options);
      if (result.data) {
  ...
  const handleVolumeByRate = rate => {
    ...
    const priceGet = Math.floor(priceGet * count) / count;
    ...
    // console.log('feesGet ==[', feesGet);
    // console.log('feesSet ==[', feesSet);
    const amountGet = Math.floor(accountFloor * priceGet * count) / count;
  ...
  return (
    <div className={classes.root}>
      ...
      <div className={classes.orderDiv}>
        ...
        <div className={classNames(classes.title, classes.titleSub)}>
          <div>
            <FormattedMessage {...messages.fees} /> ({makerFeeRate}%)
          </div>
          <div>
            <FormattedNumber
              value={fees}
      ...
      <div className={classes.orderDiv}>
        <Button className={classes.buttonOrder} onClick={handleOrderSubmit}>
          <FormattedMessage {...messages.orderSell} />
        </Button>
      </div>
    </div>
  );

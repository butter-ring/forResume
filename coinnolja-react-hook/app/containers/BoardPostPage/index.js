...
// import EditorImageUploadPop from 'components/EditorImageUploadPop';
...
import BoardPostMobile from 'components/BoardPostMobile';

import Button from '@material-ui/core/Button';
import makeSelectBoardPostPage from './selectors';
...
export function BoardPostPage({ match, history, isSignin }) {
  const classes = useStyles();
  useInjectReducer({ key: 'boardPostPage', reducer });
  useInjectSaga({ key: 'boardPostPage', saga });
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      result = await request(`/api/board`, options);
    } catch (err) {
      console.log(err);
      alert(err.data);
    } finally {
  ...
  const setInitData = async () => {
    try {
      ...
      const result = await request(
        `/api/boardmaster/${match.params.boardMasterId}`,
        options,
      );
      // console.log(result);
      if (result.data) {
        setBoardMaster(result.data);
      }
    } catch (err) {
      console.log(err);
    } finally {
  ...
  // console.log(match.params.boardType);
  if (matches) {
    return (
      <BoardPostMobile
        match={match}
...
const mapStateToProps = createStructuredSelector({
  boardPostPage: makeSelectBoardPostPage(),
